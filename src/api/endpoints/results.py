...
from src.results_manager.manager import ResultsManager, PROTEST_VOTE_KEY
...
async def get_results(date: Union[datetime.date, None] = None, db: Session = Depends(get_db)):
    ...
    return [Result(restaurant_id=res[0] if res[0] != PROTEST_VOTE_KEY else None,
                   score=res[1]) for res in results.items()]
